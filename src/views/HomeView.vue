<template>
  <div>
    ...
    <n-grid class="mt-4" x-gap="16" y-gap="16" :cols="3" item-responsive>
      <n-grid-item span="3 900:1">
        <n-card content-style="padding: 0;" class="shadow-sm">
          <div
            class="relative flex justify-between text-white bg-green-600 min-h-[5rem] pr-28 pl-4 pt-4 z-0"
          >
            <h2 class="font-bold">Welcome back!</h2>
            <img
              src="@/assets/images/welcome.gif"
              alt=""
              class="absolute right-0 -bottom-5 w-28"
            />
          </div>
          <div class="flex items-center gap-4 p-4">
            <img
              class="relative z-10 object-cover w-20 h-20 -mt-8 -ml-1 rounded-full ring-4 ring-white"
              round
              size="small"
          ...
          <ul class="flex flex-col gap-2 p-4 pt-0">
            <li class="flex items-center gap-2 w-fit" v-tippy="'Email'">
              <svg
            ...
            <li class="flex items-center gap-2 w-fit" v-tippy="'ID giảng viên'">
              <svg
                class="w-5 h-5 text-gray-400"
                fill="currentColor"
                viewBox="0 0 576 512"
              ...
              <span>{{ auth.user?.info?.info?.id ?? "---" }}</span>
            </li>
          </ul>
        </n-card>
      </n-grid-item>

      <n-grid-item span="3 900:2">
        <n-spin :show="false" class="hidden">
          <n-card content-style="padding: 0;" size="small">
            <img
              src="https://d2gg9evh47fn9z.cloudfront.net/1600px_COLOURBOX18900290.jpg"
              class="aspect-[3/1] object-cover"
              alt=""
            />

            <h1 class="px-4 font-mono text-center">
              Meet your supporter for a better teaching experience!
            </h1>
            ...
            <div class="flex flex-col gap-2 p-4">
              <div class="flex items-center justify-between gap-4">
                ...
                >
                  <img
                    src="https://cdn-icons-png.flaticon.com/512/630/630426.png"
                ...
                <n-switch
                  size="large"
                  v-model:value="isSupporterOn"
                  @update:value="toggleSupporter"
                />
              </div>
              ...
              <div class="flex items-center justify-between gap-4">
                <span
                  v-tippy="
                    'Tự động nhận xét các lớp sau khi hoàn thành buổi học'
                  "
                  class=""
                >Tự động nhận xét</span
                >
                <n-switch
                  size="small"
                  v-model:value="isAutoReviewOn"
                  :disabled="!isSupporterOn"
                />
              </div>
              ...
              <div class="flex items-center justify-between gap-4">
                <span
                  v-tippy="
                    'Đề xuất tài liệu cần thiết và các lưu ý (đã đến buổi kiểm tra, ...) trước buổi học'
                  "
                  class=""
                >Đề xuất trước buổi học</span
                >
                <n-switch
                  size="small"
                  v-model:value="isAutoPrepareOn"
                  :disabled="!isSupporterOn"
                />
              </div>
              ...
              <div class="flex items-center justify-between gap-4">
                <span
                  v-tippy="
                    'Tạo bài checkpoint và mẫu nhận xét checkpoint trước các buổi 5 và 9. Tạo nhận xét cuối khóa trước buổi 14'
                  "
                  class=""
                >Tự động tạo kiểm tra và nhận xét (CP + Final)</span
                >
                <n-switch
                  size="small"
                  v-model:value="isAutoCP"
                  :disabled="!isSupporterOn"
                />
              </div>
            </div>
        ...
        <n-card size="small" class="shadow-sm">
          <h1 class="font-bold">Đang diễn ra</h1>
          <n-divider class="!my-2"></n-divider>
          <transition name="slide-right" mode="out-in" appear>
            <p v-if="isFetchingInClass">
              <Loading1></Loading1>
            </p>
            <div v-else class="mt-4">
              <n-tag v-if="inClasses.length === 0" :bordered="false">
              ...
              >
                <ClassLogo
                  :image="inClass.course?.shortName"
                  class="w-12 h-12"
                ></ClassLogo>
                <div class="flex flex-col justify-center leading-none">
                  <h3 class="font-bold">{{ inClass.name }}</h3>
                  <span class="text-xs"
                  >Lesson {{ inClass.currentSlot + 1 }}</span
                  >
                </div>
              </router-link>
            </div>
...
export default {
  ...
  methods: {
    toggleSupporter() {
      this.messageRef.warning(
        this.isSupporterOn ? "Đã bật Supporter" : "Đã tắt Supporter",
        { duration: 1000 }
      );
    },
    fetchInClass() {
