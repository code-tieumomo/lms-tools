      ...
      <div class="mt-8">
        ...
        <div class="flex gap-4 my-4">
          <div class="flex items-center gap-1 text-xs">
            ...
            <span class="leading-3">Đang diễn ra</span>
          </div>

          <div class="flex items-center gap-1 text-xs">
            <div class="w-4 h-4 rounded bg-rose-500"></div>
            <span class="leading-3">Đã kết thúc</span>
          </div>

          <div class="flex items-center gap-1 text-xs">
            <div class="w-4 h-4 rounded bg-gray-500 opacity-50"></div>
            <span class="leading-3">Chưa diễn ra</span>
          </div>
        </div>
        <n-grid
          :x-gap="8"
        ...
        >
          <n-grid-item
            v-for="(item, index) in mindXClass.slots"
            :key="item._id"
          >
            <router-link
              :to="{ name: 'slot', params: { id, slotId: item._id } }"
            >
              <n-card
                class="[&>.n-card-header]:p-2 [&>.n-card-header]:border"
                :class="
                  item.status === 'FINISHED'
                    ? '[&>.n-card-header]:border-rose-500 text-white'
                    : item.status === 'IN_CLASS'
                    ? '[&>.n-card-header]:border-emerald-500'
                    : '[&>.n-card-header]:opacity-40'
                "
              >
                <template #header>
                  <div class="flex items-center gap-2">
                    <div
                      class="w-6 h-6 flex items-center justify-center rounded text-sm font-bold"
                      :class="
                        item.status === 'FINISHED'
                    ...
                    <span class="text-sm font-semibold">
                      {{ moment(item.startTime).format("DD-MM-Y HH:mm") }}
                    </span>
                  </div>
                </template>
              </n-card>
...
export default {
  ...
  created() {
    this.fetchClass().then(() => {
      document.title = `${this.mindXClass.name} - Daily`;
    });
  },
  methods: {
    async fetchClass() {
